...
import { ThemeToggle } from "./ThemeToggle"; // We will create this soon

// ONE-STOP-SHOP for all navigation links.
// Add a new item here, and it will appear in both desktop and mobile menus.
const navLinks = [
    { href: "/", label: "Home" },
                ...
                <nav className="hidden md:flex items-center space-x-6 text-sm font-medium">
                    {navLinks.map((link) => (
                        <Link 
                            key={link.href}
                            href={link.href}
                            className={`transition-colors focus-ring rounded-sm ${pathname === link.href 
                     ...
                     <a href="https://coff.ee/aiwritersretreat" target="_blank" rel="noopener noreferrer" className="text-ink/60 dark:text-canvas/60 hover:text-ink dark:hover:text-canvas focus-ring rounded-sm">
                        Donate
                    </a>
                </nav>
            ...
            {isMobileMenuOpen && (
                 <div className="md:hidden bg-canvas dark:bg-canvasDark border-t border-gray-200 dark:border-gray-800">
                    <nav className="px-2 pt-2 pb-4 space-y-1">
                        ...
                        <a href="https://coff.ee/aiwritersretreat" target="_blank" rel="noopener noreferrer" className="block px-3 py-2 rounded-md text-base font-medium hover:bg-gray-100 dark:hover:bg-gray-800">
                            Donate
                        </a>
                    </nav>
                </div>
